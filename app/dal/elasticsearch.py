...
class ElasticsearchCoon:
    def __init__(
        self, es_url: str, index_name: str = None, mapping: Dict[str, Any] = None
    ):
    ...
    async def initialize_index(
        self, index_name: str = None, mapping: Dict[str, Any] = None
    ) -> None:
        """Initialize the Elasticsearch index with the default mapping."""
        index_name = index_name or self.index_name
        if not index_name:
            raise ValueError(
                "Index name must be provided either during initialization or as a parameter."
            )
